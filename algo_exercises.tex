...
\begin{center}
\begin{tikzpicture}[level/.style={sibling distance=60mm/#1}]
\node [circle,draw] (z){$9S$}
  child {node [circle,draw] (a) {$2S$}
    child {node [circle,draw] (b) {nil}}
    child {node [circle,draw] (c) {nil}}
  }
  child {node [circle,draw] (d) {$17R$}
    child {node [circle,draw] (e) {$12S$}
       child {node [circle,draw] (f) {$10R$}
          child {node [circle,draw] (g) {nil}}
          child {node [circle,draw] (h) {nil}}
       }
       child {node [circle,draw] (i) {$15R$}
          child {node [circle,draw] (j) {nil}}
          child {node [circle,draw] (k) {nil}}
       }
    }
    child {node [circle,draw] (l) {$21S$}
       child {node [circle,draw] (m) {nil}}
       child {node [circle,draw] (n) {$25R$}
          child {node [circle,draw] (o) {nil}}
          child {node [circle,draw] (p) {nil}}
       }
    }
  };
\end{tikzpicture}}
...
\begin{algorithm}[H]
\SetAlgoLined
Sei P leere Liste (zum Speichern der Schnittpunkte) \\
Sei X AVL-Baum (Start und Endpunkte aller Kreise) \\
\While{X not empty} {
}
\end{algorithm} \\
